...
<html>
    <head>
        ...
        <title>Forgot password · Hapsay</title>
    ...
    <body class="min-h-screen bg-slate-50 text-slate-900">
        <main class="mx-auto max-w-md p-8 pt-20">
            ...
            <section class="mt-8 rounded-2xl bg-white p-7 shadow-sm ring-1 ring-slate-200">
                ...
                <p class="mt-2 text-sm text-slate-600">Enter your email and we’ll send a reset link if an account exists.</p>
                @if(session('status'))
                ...
                <form class="mt-6 space-y-4" method="post" action="{{ route('password.email') }}">
                    ...
                    <label class="block text-sm font-medium">
                        Email
                        <input class="mt-1 w-full rounded-xl border h-10 border-gray-500" type="email" name="email" required autofocus>
                    </label>
                    @error('email')
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <button class="rounded-xl bg-blue-700 px-4 py-3 font-semibold text-white">Email reset link</button>
                </form>
            </section>
        </main>
    </body>
